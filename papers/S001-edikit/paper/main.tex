...
synthetic two-industry economy whose identities close exactly. One of the
39 is an end-to-end check against a full third-party SAM that ships with
a companion paper not yet released, so that input is absent from the
public repository and the test skips itself: a reviewer sees 38 passed
and 1 skipped, which the README states and explains. Line
...
\begin{description}
  ...
  \texttt{toolkit/}); licence Apache-2.0; version 0.2.1, July 2026.
  \item[Archive:] Zenodo, DOI
  \href{https://doi.org/10.5281/zenodo.21442608}{10.5281/zenodo.21442608}
  (version v0.2.1; concept DOI for all versions:
...
Reusers should know the current limitations plainly. Version 0.2.1
...
instead. The toolkit does
not implement cross-entropy balancing against an arbitrary prior. Under
the standard entropy formulation with fully specified row and column
margins and compatible support, cross-entropy adjustment is closely
related to biproportional RAS \cite{robinson2001}; the unimplemented gap
is most relevant where margins are partial or additional constraints are
imposed. The automatic Eurostat workflow produces a
...
doi:10.5281/zenodo.21442608. Third-party inputs whose licences do not permit
