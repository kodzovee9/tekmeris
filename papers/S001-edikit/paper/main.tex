...
Three layers. First, a \texttt{pytest} suite (39 tests at the tagged
...
coverage is 93\,per\,cent overall (84--99\,per\,cent per implemented
...
cells exact, the remaining two both reflecting a single unrecorded
...
\begin{description}
  ...
  \texttt{toolkit/}); licence Apache-2.0; version 0.2.0, July 2026.
  % !! STALE: this is the v0.1.5 VERSION DOI. The manuscript now describes
  % !! v0.2.0. Cut a v0.2.0 release on the public repo so Zenodo mints a new
  % !! version DOI, then replace the two occurrences here and in the Data
  % !! Accessibility Statement. The concept DOI below is version-independent
  % !! and stays as is. DO NOT SUBMIT until this is resolved.
  \item[Archive:] Zenodo, DOI
  \href{https://doi.org/10.5281/zenodo.21428107}{10.5281/zenodo.21428107}
  (version v0.1.5; concept DOI for all versions:
...
Reusers should know the current limitations plainly. Version 0.2.0
...
directly, while standard RAS requires a non-negative seed (the
pipeline's sign-normalisation rule handles the negative cells that arise
in practice under RAS, and refuses silently mishandling others). It does
not implement cross-entropy balancing against an arbitrary prior; note
that where both row and column margins are fixed, cross-entropy to a
prior coincides with RAS or GRAS seeded with that prior, so the gap bears
only on partial-margin problems. The automatic Eurostat workflow produces a
...
Natural extensions, deliberately scoped out of version 0.2.0, include
...
doi:10.5281/zenodo.21428107. Third-party inputs whose licences do not permit
