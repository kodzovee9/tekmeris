...
removed by better data. The first is what the method can see. Every
finding reported here was detected because it left an arithmetic
fingerprint: an identity refusing to close, a grid returning one survivor,
a pair of gaps too symmetric to be coincidence. Divergences that leave no
such trace---compensating errors, an adjustment that lands on a plausible
value, an error shared between the artefact and the sources it is checked
against---pass every test in this framework silently. The audit therefore
establishes reproducibility of a matrix from its stated record, which is
not the same as its correctness; no number in this paper is validated
against an independent measurement of the same quantity, and none of our
three cases could have been.

The second is that the framework logs without judging. It reports every
adjustment factor, coverage share, and residual, but it does not encode a
threshold above which a matrix should be rejected, and we do not propose
one here: the tolerable magnitude of a balancing adjustment depends on the
account, the country, and the use to which the SAM will be put, and a
number chosen to fit three cases would be arbitrary precision dressed as a
standard. What the framework changes is that the question becomes
answerable by a reader---the factors and residuals are published, so a
user can apply their own threshold---rather than invisible. A discipline
that wants rejection rules will need them agreed by producers, not
asserted by one toolkit.
