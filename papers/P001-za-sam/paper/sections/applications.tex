...
2019 provisional).}
\label{tab:kenya}
\input{tables/tab_kenya}
\end{table}

\emph{Cell-level audit is blocked by publication practice, not by
...
as condensed print tables. The pipeline that reproduced South Africa's
production block exactly would run unchanged on the Kenyan detail; what it
lacks is a machine-readable input, not a method. That boundary---audit
feasible in principle, blocked by format---defines the middle of the
replicability spectrum.
...
four independent series. Table~\ref{tab:netherlands} reports the residual
...
step: it converges to a maximum residual below EUR~0.01~million with all
70 adjustment factors inside $[0.983, 1.028]$, concentrated on the
rest-of-world cells---exactly where the attributed residuals sat---and
every factor ships with the matrix. The balancing choices (the treatment
...
adjustment factors in $[0.983, 1.028]$.}
\label{tab:netherlands}
\input{tables/tab_netherlands}
\end{table}
...
Appendix~\ref{app:transfer} reports the results. The marginal cost of a verified macro SAM, conditional on the
toolkit, has fallen to approximately the cost of the data's publication
format---which is the replicability spectrum stated as a price.
