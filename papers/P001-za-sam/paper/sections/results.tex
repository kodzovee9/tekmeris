...
Balanced SAM (v0.2) & max account imbalance & R7m; RAS factors in $[0.91, 1.14]$ \\
\midrule
Non-public inputs & count; priced effect of proxy & 1; mtx $+16.1$\%, stx $-1.5$\% \\
Divergences record vs.\ data & detected deterministically & 8 (Section~\ref{sec:discussion}) \\
\bottomrule
\end{tabularx}
\end{table}
...
every category (households 93/93 commodities, government, exports 97/97,
fixed capital formation 21/21, and inventories-plus-residual 102/102,
...
(maximum $|$row$-$column$|$ gap below R0.001~million across its 209
accounts), and its Table~2 macro SAM equals, cell for cell, the kind-level
aggregation we compute independently from the distributed workbook.
...
0.139\,per\,cent. Getting there required three corrections that are
themselves results:
...
$\times$ carrying-value columns identifies \emph{2019 revised, closing
values} as the construction basis (median share deviation 0.18~percentage
...
\textbf{Households.} Two dictionary-free anchors identify the benchmark's
group definition as \emph{household-level} expenditure deciles (top decile
...
0.74pp (max 1.9pp), while per-capita grouping fits markedly worse. The
consumption anchor deviates by up to 5.9pp under either definition---as the
note itself explains, its consumption shares were carried over from an
earlier SAM rather than recomputed from the LCS: inherited provenance that
a fresh, documented build cannot and should not reproduce.
...
confined to $[0.91, 1.14]$, leaves every other account within
